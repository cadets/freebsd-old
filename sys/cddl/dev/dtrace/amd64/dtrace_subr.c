...
dtrace_gla2hpa(struct vm_guest_paging *paging, caddr_t gla, caddr_t *hpa)
{
 ...
 restart:
	...
	while (--nlevels >= 0) {
		/*
		 * Here we zero the lower and upper 12 bits.
		 */
		ptpphys >>= 12; ptpphys <<= 24; ptpphys >>= 12;
		/*
		 * We have to do a page table lookup.
		 */
		if (ptpbase == NULL)
			return (EINVAL);
		...
		if ((pte & PG_V) == 0 ||
			...
			/*
			 * TODO(dstolfa): Page fault.
			 * XXX(dstolfa): Do we want the writable check and
			 * usermode check here at all?
			 */
			return (EFAULT);
		}

		/*
		 * Have we accessed the page table?
		 */
		if ((pte & PG_A) == 0) {
			/*
			 * XXX(dstolfa): For now we just set it and restart.
			 */
