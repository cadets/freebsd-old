...
LLVM_C= ${CC} -emit-llvm ${CFLAGS:N-O*} -c ${.IMPSRC} -o ${.TARGET}
LLVM_S= ${NORMAL_S}
LLVM_IR=${LLC} -code-model=kernel -filetype=obj \
	${CFLAGS:M-O} ${.IMPSRC} -o ${.TARGET}
...
LLVM_INSTRUMENT= ${OPT} ${LLVM_INSTR_FLAGS} ${.IMPSRC} -o ${.TARGET}
